  ...
  return (
    <div>
      ...
      <form onSubmit={buscar} className="mb-4 flex flex-wrap items-end gap-3 rounded-2xl border-2 border-[#e2e8f0] dark:border-slate-700 bg-white dark:bg-[#141a2e] p-4">
        <div>
          <label className="block text-xs font-bold text-[#64748b] dark:text-slate-400">Proveedor</label>
          <input
            type="text"
            value={borrador.proveedor}
            onChange={(e) => setBorrador((b) => ({ ...b, proveedor: e.target.value }))}
            placeholder="Nombre del proveedor..."
            className="mt-1 w-48 rounded-xl border-2 border-[#e2e8f0] dark:border-slate-700 bg-white dark:bg-[#141a2e] px-3 py-2.5 text-sm font-medium text-[#1e293b] dark:text-slate-100 outline-none transition-all focus:border-pink-500 focus:ring-4 focus:ring-pink-100"
          />
        </div>
        <div>
          ...
          <input
            ...
            className="mt-1 w-36 rounded-xl border-2 border-[#e2e8f0] dark:border-slate-700 bg-white dark:bg-[#141a2e] px-3 py-2.5 text-sm font-medium text-[#1e293b] dark:text-slate-100 outline-none transition-all focus:border-pink-500 focus:ring-4 focus:ring-pink-100"
          />
        </div>
        <div>
        ...
        <button
          type="button"
          onClick={filtrarHoy}
          className="mt-5 rounded-xl border-2 border-[#e2e8f0] dark:border-slate-700 bg-white dark:bg-[#141a2e] px-4 py-2.5 text-sm font-bold text-pink-600 transition-all hover:bg-pink-50 active:scale-95 dark:hover:bg-slate-800"
        >
          Hoy
        </button>
        <button
          type="submit"
          className="rounded-md bg-pink-600 px-6 py-2.5 text-sm font-bold text-white shadow-sm shadow-pink-500/30 transition-all hover:bg-pink-700 active:scale-95"
        >
          🔍 Buscar
        </button>
        {hayFiltros && (
      ...
      <div className="overflow-hidden rounded-3xl border-2 border-[#e2e8f0] dark:border-slate-700 bg-white dark:bg-[#141a2e] shadow-lg shadow-slate-500/5">
        {ordenes.length === 0 ? (
          <p className="p-12 text-center text-sm font-medium text-[#64748b] dark:text-slate-400">Todavía no hay órdenes de compra.</p>
        ) : filtradas.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-[13.5px]">
              ...
              <tbody>
                {filtradas.map((o) => (
                  <tr key={o.id} className="border-b border-[#f1f5f9] dark:border-slate-800 text-[#1e293b] dark:text-slate-100 transition-colors hover:bg-pink-50/40">
                    ...
                    <td className="px-6 py-4 text-[#64748b] dark:text-slate-400">
                      {TIPO_DOCUMENTO_LABELS[o.tipo_documento] ?? o.tipo_documento}
                      {o.documento_serie && o.documento_numero && (
                        <span className="ml-1 font-semibold text-[#1e293b] dark:text-slate-100">
                          {o.documento_serie}-{o.documento_numero}
                        </span>
                      )}
                    </td>
