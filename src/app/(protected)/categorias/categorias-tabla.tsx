...
type CategoriaRow = { id: number; nombre: string; productos: number }
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-[#1e293b] dark:text-slate-100">🏷️ Categorías</h1>
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Filtrar..."
              className="rounded-2xl border-2 border-[#e2e8f0] dark:border-slate-700 bg-white dark:bg-[#141a2e] py-2.5 pr-4 pl-10 text-sm font-medium text-[#1e293b] dark:text-slate-100 outline-none transition-all focus:border-violet-500 focus:ring-4 focus:ring-violet-100"
            />
          </div>
          <Link
            href="/categorias/nueva"
            className="flex items-center gap-2 rounded-2xl bg-gradient-to-r from-violet-500 to-purple-500 px-6 py-3 text-sm font-bold text-white shadow-lg shadow-violet-500/30 transition-all hover:-translate-y-0.5 hover:shadow-xl hover:shadow-violet-500/40"
          >
      ...
      <div className="mt-6 overflow-hidden rounded-3xl border-2 border-[#e2e8f0] dark:border-slate-700 bg-white dark:bg-[#141a2e] shadow-lg shadow-slate-500/5">
        {categorias.length === 0 ? (
          <p className="p-12 text-center text-sm font-medium text-[#64748b] dark:text-slate-400">
            No hay categorías todavía. Usa el botón{' '}
            <Link href="/categorias/nueva" className="font-bold text-violet-600">
              Nueva categoría
            </Link>
            .
          </p>
          ...
          <table className="w-full text-left text-[13.5px]">
            ...
            <tbody>
              {filtradas.map((c) => (
                <tr key={c.id} className="border-b border-[#f1f5f9] dark:border-slate-800 text-[#1e293b] dark:text-slate-100 transition-colors hover:bg-violet-50/40">
                  ...
                  <td className="px-6 py-4 text-right whitespace-nowrap">
                    <Link
                      ...
                      className="inline-flex h-9 w-9 items-center justify-center rounded-xl text-[#64748b] dark:text-slate-400 transition-all hover:bg-violet-100 hover:text-violet-600"
                      title="Editar"
                    >
