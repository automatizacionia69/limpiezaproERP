...
export type EntradaCabeceraRow = {
  id: number
  numero: string
  fecha: string
  motivo: string
  ...
  return (
    <div className="overflow-hidden rounded-3xl border-2 border-[#e2e8f0] dark:border-slate-700 bg-white dark:bg-[#141a2e] shadow-lg shadow-slate-500/5">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b-2 border-[#f1f5f9] dark:border-slate-800 px-6 py-5">
        <h2 className="text-base font-extrabold text-[#1e293b] dark:text-slate-100">🕒 Entradas registradas</h2>
        {entradas.length > 0 && (
          <div className="relative">
            ...
            <input
              ...
              placeholder="Filtrar..."
              className="rounded-2xl border-2 border-[#e2e8f0] dark:border-slate-700 bg-white dark:bg-[#141a2e] py-2 pr-4 pl-10 text-sm font-medium text-[#1e293b] dark:text-slate-100 outline-none transition-all focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100"
            />
          </div>
        )}
      </div>
      ...
      {entradas.length === 0 ? (
        <p className="p-12 text-center text-sm font-medium text-[#64748b] dark:text-slate-400">Todavía no hay entradas registradas.</p>
      ) : filtradas.length === 0 ? (
        <p className="p-12 text-center text-sm font-medium text-[#64748b] dark:text-slate-400">Ninguna entrada coincide con “{filtro}”.</p>
      ) : (
          ...
          <table className="w-full text-left text-[13.5px]">
            <thead>
              <tr className="border-b-2 border-[#f1f5f9] dark:border-slate-800 bg-[#f8fafc] dark:bg-slate-800/60 text-[#64748b] dark:text-slate-400">
                ...
                <th className="px-6 py-4 font-bold">F. Ingreso</th>
            ...
            <tbody>
              {filtradas.map((e) => {
                ...
                return (
                  <tr
                    key={e.id}
                    className="border-b border-[#f1f5f9] dark:border-slate-800 text-[#1e293b] dark:text-slate-100 transition-colors hover:bg-emerald-50/40"
                  >
