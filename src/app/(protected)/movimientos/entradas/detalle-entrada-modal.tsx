...
import { obtenerDetalleEntrada, type DetalleEntrada } from './actions'
...
/** Popup con el detalle de una entrada — reemplaza la navegación a una página aparte. */
export function DetalleEntradaModal({ entradaId, onCerrar }: { entradaId: number | null; onCerrar: () => void }) {
  ...
  useEffect(() => {
    ...
    let cancelado = false
    obtenerDetalleEntrada(entradaId).then((resultado) => {
      if (cancelado) return
  ...
  return (
    <Modal abierto={entradaId !== null} onClose={onCerrar} className="max-w-3xl">
      {error && (
      ...
      {!detalle && !error && (
        <p className="py-10 text-center text-sm font-medium text-[#94a3b8] dark:text-slate-500">Cargando entrada...</p>
      )}
        ...
        <>
          ...
          <div className="flex items-start justify-between gap-6 border-b-2 border-[#f1f5f9] dark:border-slate-800 pb-5">
            <div className="flex items-start gap-4">
              <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-emerald-100 text-2xl dark:bg-emerald-950/40">
                📥
              </span>
              <div>
                <h2 className="text-xl font-extrabold text-[#1e293b] dark:text-slate-100">Entrada de mercadería</h2>
            ...
            <div className="w-40 shrink-0 rounded-xl border-2 border-[#1e293b] dark:border-slate-600 p-3 text-center">
              <p className="text-xs font-extrabold tracking-wide text-[#1e293b] dark:text-slate-100 uppercase">Entrada</p>
              <p className="mt-1 text-base font-extrabold text-emerald-600">{detalle.numero}</p>
              <span
          ...
          <div className="mt-5 grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
            <div className="rounded-xl border border-[#e2e8f0] dark:border-slate-700 p-4">
              <p className="text-[10px] font-bold tracking-wide text-[#94a3b8] dark:text-slate-500 uppercase">Datos de la entrada</p>
              <div className="mt-1.5 space-y-0.5 divide-y divide-dashed divide-[#e2e8f0] dark:divide-slate-800">
                <Fila etiqueta="Fecha de ingreso" valor={new Date(`${detalle.fecha}T00:00:00`).toLocaleDateString('es-PE')} />
          ...
          <div className="mt-4 flex justify-end">
            <div className="w-64 space-y-1.5 rounded-xl border border-[#e2e8f0] dark:border-slate-700 p-4">
              ...
              <p className="flex justify-between border-t-2 border-[#1e293b] dark:border-slate-600 pt-2 text-lg font-extrabold text-emerald-600">
