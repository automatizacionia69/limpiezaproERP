...
export type ItemDetalleEntrada = {
  id: number
  cantidad: number
...
export type DetalleEntrada = {
  ...
  items: ItemDetalleEntrada[]
}

/** Detalle de una entrada para el popup "Ver" de la tabla — misma consulta que antes vivía en /movimientos/entradas/[id]. */
export async function obtenerDetalleEntrada(id: number): Promise<{ detalle: DetalleEntrada } | { error: string }> {
  if (!(await tienePermiso('movimientos'))) {
    return { error: 'No tienes permiso para esta acción.' }
  }
