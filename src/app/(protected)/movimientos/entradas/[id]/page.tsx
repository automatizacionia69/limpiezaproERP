...
export default async function VerEntradaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <Link href="/movimientos/entradas" className="text-sm font-bold text-[#64748b] dark:text-slate-400 hover:text-emerald-600">
        ← Volver a Entradas
      </Link>
      ...
      <div className="mx-auto mt-4 max-w-4xl rounded-3xl border-2 border-[#e2e8f0] dark:border-slate-700 bg-white dark:bg-[#141a2e] p-8 shadow-lg shadow-slate-500/5 sm:p-10">
        ...
        <div className="flex items-start justify-between gap-6 border-b-2 border-[#1e293b] dark:border-slate-600 pb-5">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h1 className="text-xl font-extrabold text-[#1e293b] dark:text-slate-100">Entrada de mercadería</h1>
          ...
          <div className="w-52 shrink-0 rounded-xl border-2 border-[#1e293b] dark:border-slate-600 p-4 text-center">
            <p className="text-sm font-extrabold tracking-wide text-[#1e293b] dark:text-slate-100 uppercase">Entrada</p>
            <p className="mt-1 text-lg font-extrabold text-emerald-600">{cabecera.numero}</p>
            <span
        ...
        <div className="mt-5 grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
          <div className="rounded-xl border border-[#e2e8f0] dark:border-slate-700 p-4">
            ...
            <div className="mt-1.5 space-y-0.5 divide-y divide-dashed divide-[#e2e8f0] dark:divide-slate-800">
              <Fila etiqueta="Fecha de ingreso" valor={new Date(`${cabecera.fecha}T00:00:00`).toLocaleDateString('es-PE')} />
