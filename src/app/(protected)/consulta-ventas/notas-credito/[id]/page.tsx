...
export default async function NotaCreditoPage({
  ...
  const { data: nota } = await supabase
    .from('notas_credito')
    .select(
      'id, numero, motivo, anula_operacion, monto, observacion, creado_en, comprobante_id, comprobantes(numero, tipo, total, estado, clientes(nombre, documento, direccion))'
    )
    .eq('id', id)
    .single()
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between print:hidden">
        ...
        <ImprimirBoton className="flex items-center gap-2 rounded-md bg-gradient-to-r from-red-500 to-rose-600 px-6 py-3 text-sm font-bold text-white shadow-lg shadow-red-500/30 transition-all active:scale-95">
          🖨️ Imprimir
        </ImprimirBoton>
      </div>
      ...
      <div className="mx-auto max-w-3xl rounded-3xl border-2 border-[#e2e8f0] dark:border-slate-700 bg-white dark:bg-[#141a2e] p-10 shadow-lg shadow-slate-500/5 print:max-w-none print:rounded-none print:border-0 print:p-0 print:shadow-none">
        <div className="flex items-start justify-between gap-6 border-b-2 border-[#1e293b] dark:border-slate-600 pb-5">
          ...
          <div className="w-56 shrink-0 rounded-xl border-2 border-red-600 p-4 text-center">
            {configuracion?.ruc && <p className="text-xs font-bold text-[#1e293b] dark:text-slate-100">RUC {configuracion.ruc}</p>}
            <p className="mt-1 text-sm font-extrabold tracking-wide text-red-600 uppercase">Nota de crédito</p>
            <p className="mt-1 text-lg font-extrabold text-red-600">{nota.numero}</p>
          </div>
        </div>
        ...
        <p className="mt-5 rounded-xl bg-red-50 px-4 py-2.5 text-sm text-red-700">
          ...
          {nota.anula_operacion && ' · Anula la operación'}
        </p>
            ...
            <tbody>
              {detalles.map((d) => (
                <tr key={d.id} className="border-b border-[#f1f5f9] dark:border-slate-800">
                  ...
                  <td className="py-2.5 text-right">S/ {(Number(d.cantidad) * Number(d.precio_unitario)).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <p className="mt-10 text-center text-[11px] text-[#94a3b8] dark:text-slate-500">
          Nota de crédito generada por LimpiezaPro ERP — sin validez tributaria (sin integración con SUNAT).
        </p>
      </div>
    </div>
  )
